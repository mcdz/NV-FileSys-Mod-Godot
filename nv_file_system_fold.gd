...
const PATH_CONFIG: String = "res://addons/nv_fsys_fold/config.cfg"
...
func _enter_tree() -> void:
	...
	for u in unit.keys():
		if unit[u] == null:
			print('{}\n{}{}'.format(['nv_fsys_fold', "Failed Creating a reference for : ", u], '{}'))
			return
	
	for node in file_system_vsplit.get_children():
		node.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	file_system_vsplit.get_child(0).size_flags_stretch_ratio = 1
